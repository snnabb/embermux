/* EmberMux Admin Panel */
(function(){
'use strict';
const TK='embermux_token';
let cached=[];
...
function valUpstream(){
  const e=[];
  if(!document.getElementById('uf-name').value.trim())e.push('名称不能为空');
  const u=document.getElementById('uf-url').value.trim();
  if(!u)e.push('上游地址不能为空');else if(!isURL(u))e.push('上游地址需以 http(s):// 开头');
  // validate playback URLs
  document.querySelectorAll('.pb-input').forEach((inp,i)=>{const v=inp.value.trim();if(v&&!isURL(v))e.push('播放回源地址 '+(i+1)+' 格式无效')});
  if(document.getElementById('uf-authType').value==='password'){if(!document.getElementById('uf-username').value.trim())e.push('用户名不能为空')}
  else{if(!document.getElementById('uf-apiKey').value.trim())e.push('API Key 不能为空')}
  return e;
}
function valProxy(){const e=[];if(!document.getElementById('pf-name').value.trim())e.push('名称不能为空');const u=document.getElementById('pf-url').value.trim();if(!u)e.push('代理地址不能为空');else if(!isURL(u)&&!/^socks5?:\/\//i.test(u))e.push('代理地址格式无效');return e}
function valSettings(){
  const e=[];if(!document.getElementById('sf-serverName').value.trim())e.push('服务名称不能为空');
  [['sf-timeout-api','API超时'],['sf-timeout-global','聚合超时'],['sf-timeout-login','登录超时'],['sf-timeout-healthCheck','健康检查超时'],['sf-timeout-healthInterval','检查间隔']].forEach(([id,l])=>{const v=+document.getElementById(id).value;if(!v||v<=0)e.push(l+' 必须为正整数')});
  if(document.getElementById('sf-newPassword').value&&!document.getElementById('sf-currentPassword').value)e.push('修改密码需要填写当前密码');return e;
}
...
async function loadDash(){const r=await api('/status');if(!r)return;const d=await r.json();
  ...
  sg.innerHTML=d.upstream.map(u=>'<div class="card"><div class="card-header"><span class="card-title"><span class="dot '+(u.online?'dot-online':'dot-offline')+'"></span>'+esc(u.name)+'</span>'+pBadge(u.playbackMode)+'</div><div class="card-body"><p>'+esc(u.host)+'</p></div></div>').join('')}
function sc(l,v,c){return'<div class="stat-card"><div class="stat-label">'+esc(l)+'</div><div class="stat-value '+c+'">'+esc(String(v))+'</div></div>'}
...
async function doReorder(f,t){const r=await api('/upstream/reorder',{method:'POST',body:JSON.stringify({fromIndex:f,toIndex:t})});if(!r)return;if(!r.ok){toast('排序失败','error');return}toast('排序已更新','success');loadUp();loadDash()}

// Upstreams
async function loadUp(){
  const r=await api('/upstream');if(!r)return;const list=await r.json();cached=list||[];
  const c=document.getElementById('upstream-list');
  ...
  c.innerHTML=list.map(u=>{
    ...
    let pbInfo='';
    const hosts=[u.streamingUrl||'',...(u.streamHosts||[])].filter(Boolean);
    if(hosts.length===1)pbInfo='<p>播放回源：'+esc(hosts[0])+'</p>';
    else if(hosts.length>1)pbInfo='<p>播放回源：'+hosts.length+' 个地址</p>';
    return'<div class="card" data-index="'+u.index+'" draggable="true"><div class="card-header"><span class="card-title">'+DH+'<span class="dot '+(u.online?'dot-online':'dot-offline')+'"></span>'+esc(u.name)+'</span>'+pBadge(u.playbackMode)+'</div><div class="card-body"><p>地址：'+esc(u.url)+'</p><p>认证：'+(u.authType==='apiKey'?'API Key':'用户名/密码')+(u.username?' ('+esc(u.username)+')':'')+'</p>'+pbInfo+'<p>UA 伪装：'+esc(uLabel(u.spoofClient))+'</p></div><div class="card-footer"><button class="btn btn-ghost btn-sm" onclick="EM.editUp('+u.index+')">编辑</button><button class="btn btn-ghost btn-sm" onclick="EM.reconUp('+u.index+')">重连</button><button class="btn btn-ghost btn-sm" onclick="EM.diag(this,'+u.index+')">诊断</button><button class="btn btn-danger btn-sm" onclick="EM.delUp('+u.index+')">删除</button></div></div>'}).join('')}

// Multi-playback URL list helper
function buildPlaybackList(container, hosts) {
  function render() {
    container.innerHTML = hosts.map((v,i) =>
      '<div class="pb-row"><input type="text" class="form-group pb-input" value="'+esc(v)+'" placeholder="'+(i===0?'主播放回源地址':'额外播放回源地址')+'" style="padding:.65rem .85rem;font-size:.9rem;background:var(--bg-tertiary);border:1px solid var(--border);border-radius:var(--radius-sm);color:var(--text-primary);outline:none">'+(hosts.length>1?'<button type="button" class="btn btn-danger btn-sm pb-remove" data-i="'+i+'">删除</button>':'')+'</div>'
    ).join('');
    container.querySelectorAll('.pb-remove').forEach(b => b.onclick = () => { hosts.splice(+b.dataset.i, 1); if(!hosts.length)hosts.push(''); render(); });
    container.querySelectorAll('.pb-input').forEach((inp,i) => inp.oninput = () => { hosts[i] = inp.value; });
  }
  render();
  return { addRow() { hosts.push(''); render(); const inputs = container.querySelectorAll('.pb-input'); if(inputs.length) inputs[inputs.length-1].focus(); } };
}
...
function upForm(data) {
  ...
  return`
    <div class="form-group"><label>名称 <span class="required">*</span></label><input id="uf-name" value="${esc(d.name||'')}"></div>
    ...
    <div id="uf-ak" class="hidden"><div class="form-group"><label>API Key <span class="required">*</span></label><input id="uf-apiKey" value=""></div></div>
...
let pbListCtrl=null;
function initPlaybackList(data) {
  const d=data||{};
  let hosts=[];
  if(d.streamingUrl)hosts.push(d.streamingUrl);
  if(d.streamHosts&&d.streamHosts.length)hosts=hosts.concat(d.streamHosts);
  if(!hosts.length)hosts=[''];
  const container=document.getElementById('uf-pb-list');
  pbListCtrl=buildPlaybackList(container, hosts);
  document.getElementById('uf-pb-add').onclick=()=>pbListCtrl.addRow();
}
...
function collectUp(){
  ...
  const body={name:document.getElementById('uf-name').value,url:document.getElementById('uf-url').value,
    ...
    followRedirects:document.getElementById('uf-followRedirects').checked,priorityMetadata:document.getElementById('uf-priorityMeta').checked,
    proxyId:document.getElementById('uf-proxyId').value||''};
  // Collect playback URLs
  const allPB=[];document.querySelectorAll('.pb-input').forEach(inp=>{const v=inp.value.trim();if(v)allPB.push(v)});
  body.streamingUrl=allPB.length>0?allPB[0]:'';
  body.streamHosts=allPB.length>1?allPB.slice(1):[];
...
function showAddUp(){
  openModal('添加上游',upForm(),'<button class="btn btn-ghost" onclick="EM.closeModal()">取消</button><button class="btn btn-primary" id="uf-save">保存</button>');
  EM.togAuth();initPlaybackList();
  const b=document.getElementById('uf-save');
  b.onclick=withLoading(b,async()=>{if(!showErr(valUpstream()))return;const r=await api('/upstream',{method:'POST',body:JSON.stringify(collectUp())});if(!r)return;if(!r.ok){const e=await r.json();toast(e.error||'添加失败','error');return}closeModal();toast('上游添加成功','success');loadUp();loadDash()});
}

async function editUp(idx){
  const r=await api('/upstream');if(!r)return;const list=await r.json();cached=list||[];
  const u=list.find(x=>x.index===idx);if(!u){toast('未找到上游','error');return}
  openModal('编辑上游',upForm(u),'<button class="btn btn-ghost" onclick="EM.closeModal()">取消</button><button class="btn btn-primary" id="uf-save">保存</button>');
  EM.togAuth();initPlaybackList(u);
  const b=document.getElementById('uf-save');
  b.onclick=withLoading(b,async()=>{if(!showErr(valUpstream()))return;const r=await api('/upstream/'+idx,{method:'PUT',body:JSON.stringify(collectUp())});if(!r)return;if(!r.ok){const e=await r.json();toast(e.error||'保存失败','error');return}closeModal();toast('上游更新成功','success');loadUp();loadDash()});
}
...
async function reconUp(idx){
  const btn=event&&event.target&&event.target.closest('.btn');
  const fn=async()=>{const r=await api('/upstream/'+idx+'/reconnect',{method:'POST'});if(!r)return;toast('已触发重连','success');setTimeout(()=>{loadUp();loadDash()},1500)};
...
// Diagnostics
async function diag(btn,idx){
  const card=btn.closest('.card');let p=card.querySelector('.diag-panel');if(p){p.remove();return}
  let u=cached.find(x=>x.index===idx);
  if(!u){const r=await api('/upstream');if(!r)return;cached=await r.json();u=cached.find(x=>x.index===idx)}
  ...
  rows.push(dr('UA 伪装',esc(uLabel(u.spoofClient))));
  rows.push(dr('播放模式',esc(pLabel(u.playbackMode))));
  const hosts=[u.streamingUrl||'',...(u.streamHosts||[])].filter(Boolean);
  if(hosts.length)rows.push(dr('播放回源',hosts.map(h=>esc(h)).join('<br>')));
...
async function loadPx(){const r=await api('/proxies');if(!r)return;const list=await r.json();const c=document.getElementById('proxy-list');
  ...
  c.innerHTML=list.map(p=>'<div class="card" data-id="'+esc(p.id)+'"><div class="card-header"><span class="card-title">'+esc(p.name)+'</span></div><div class="card-body"><p>地址：'+esc(p.url)+'</p></div><div class="card-footer"><button class="btn btn-ghost btn-sm" onclick="EM.testPx(\''+esc(p.id)+'\')">测试</button><button class="btn btn-danger btn-sm" onclick="EM.delPx(\''+esc(p.id)+'\')">删除</button></div></div>').join('')}

function showAddPx(){
  openModal('添加代理','<div class="form-group"><label>名称 <span class="required">*</span></label><input id="pf-name" placeholder="My Proxy"></div><div class="form-group"><label>代理地址 <span class="required">*</span></label><input id="pf-url" placeholder="http://127.0.0.1:7890"></div>','<button class="btn btn-ghost" onclick="EM.closeModal()">取消</button><button class="btn btn-primary" id="pf-save">保存</button>');
  const b=document.getElementById('pf-save');
  b.onclick=withLoading(b,async()=>{if(!showErr(valProxy()))return;const r=await api('/proxies',{method:'POST',body:JSON.stringify({name:document.getElementById('pf-name').value,url:document.getElementById('pf-url').value})});if(!r)return;if(!r.ok){const e=await r.json();toast(e.error||'添加失败','error');return}closeModal();toast('代理添加成功','success');loadPx()});
}
...
async function loadSet(){const r=await api('/settings');if(!r)return;const s=await r.json();const modes=[['proxy','代理中转'],['direct','直连分流'],['redirect','重定向跟随']];const t=s.timeouts||{};
  ...
  const b=document.getElementById('sf-save');b.onclick=withLoading(b,saveSet)}

async function saveSet(){if(!showErr(valSettings()))return;
  const body={serverName:document.getElementById('sf-serverName').value,playbackMode:document.getElementById('sf-playbackMode').value,adminUsername:document.getElementById('sf-adminUsername').value,
    timeouts:{api:+document.getElementById('sf-timeout-api').value,global:+document.getElementById('sf-timeout-global').value,login:+document.getElementById('sf-timeout-login').value,healthCheck:+document.getElementById('sf-timeout-healthCheck').value,healthInterval:+document.getElementById('sf-timeout-healthInterval').value}};
  ...
  const r=await api('/settings',{method:'PUT',body:JSON.stringify(body)});if(!r)return;if(!r.ok){const e=await r.json();toast(e.error||'保存失败','error');return}toast('设置已保存','success');loadSet()}
...
function renderLogs(){const search=(document.getElementById('log-search').value||'').toLowerCase(),level=document.getElementById('log-filter').value,viewer=document.getElementById('log-viewer');
  let html='';for(const e of logsData){const ts=e.timestamp||'',lv=e.level||'',msg=e.message||'';const line=ts+' ['+lv+'] '+msg;
    if(search&&!line.toLowerCase().includes(search))continue;if(level&&lv!==level)continue;
    let cls='log-debug';if(lv==='ERROR')cls='log-error';else if(lv==='WARN')cls='log-warn';else if(lv==='INFO')cls='log-info';
    html+='<span class="log-line '+cls+'"><span class="log-ts">'+esc(ts)+'</span> <span class="log-lv">['+esc(lv)+']</span> '+esc(msg)+'</span>\n'}
...
window.EM={editUp,delUp,reconUp:reconUp,diag,togAuth,delPx,testPx,closeModal};
